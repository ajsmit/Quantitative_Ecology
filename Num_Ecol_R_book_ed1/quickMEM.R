...
#   vectors after Borcard and Legendre (2002). This function, quickMEM, 
...
#   correlation: they are the ones whose eigenvalue islarger than Moran's I
...
#   therefore, positive Moran's I) are computed.
...
#        is not appropriate). Beware: provide a thresh value very slightly 
#        larger than the actual desired distance. For instance, if this 
#        distance is 3.0, type thresh=3.00001.
...
dbMEM.comput <- dbmem(XY, thresh=thresh, silent=FALSE)
# Replace silent=FALSE by use of give.thresh ?
...
if(detrend==TRUE){
   if(temp2.test[1, ncol(temp2.test)] <= alpha) {
      ...
      cat("\nThe data have been detrended prior to dbMEM analysis.")
                                                }
    else{
      cat("\n-------------------------------------------------------")
...
cat("\n-------------------------------------------------------\n")
# cat("\nThe truncation value used for dbMEM building is",dmin,"\n")
